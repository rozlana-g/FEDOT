...
def prepare_data(task, label="GS10"):
...
def evaluate_fedot(forecast_length):
    task = Task(TaskTypesEnum.ts_forecasting,
                TsForecastingParams(forecast_length=forecast_length))

    label = "EXCAUS"
    input_data = prepare_data(task, label=label)
    train_input, predict_input = train_test_data_setup(input_data)
    # Init model for the time series forecasting
    model = Fedot(problem='ts_forecasting', task_params=task.task_params, timeout=0.5)
